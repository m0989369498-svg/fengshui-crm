...
const sz = analyzeShuzi('0900000123');
assert(sz.dominant_star && sz.lucky_pct >= 0);
ok(`數字易經 0900000123 → 主導 ${sz.dominant_star}, 吉 ${sz.lucky_pct}%`);
...
const xm = computeXingming('測試用戶');
assert(xm && xm.sancai.verdict && xm.ren_wx);
ok(`姓名學 測試用戶 → 三才${xm.sancai.verdict}, 人格${xm.ren_wx}型`);
...
const f = extractFeatures({ name: '測試用戶', gender: 'male', birth: { year: 1992, month: 10, day: 1, hour: 16, minute: 44, city: '台北' }, phone: '0900000123' });
...
const r = await generateStrategy({ name: '測試用戶', phone: '0900000123' }, { salesQuestion: '高價猶豫', lang: 'zh-Hant' });
